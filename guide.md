...
Create file `train_unsupervised.py`
1. Load the manually scored dataset. Remember, we only use the score for filtering known "good" data:
```python
import pandas as pd
import numpy as np
import matplotlib.pyplot as plt
...
2. Train the model with the filtered data, then run the prediction on the whole dataset.
```python
...
plt.show()
```
Run with `python .\train_unsupervised.py`<br />
Upon running, your visualization will illustrate the manual quality score vs. the model's predicted "anomaly score" - which is then used to classify *normal* (blue) vs. *anomalous* (red) behavior. Ideally, you have a strong correlations between **high scores : normal** and **low scores : anomalous** - which shows as clustering in the top right (blue) and bottom left (red).

#### Isolation Forests

***More Trees??*** Yes, an isolation forest similarly uses trees for inference, but they are built in a much different way:
> Unlike with the (supervised) regressor trees, isolation forest trees are generated by randomly splitting or partitioning the datasets until points are isolated. By nature, outliers will be isolated in **fewer** partitions, and thus be *shallower* in the tree than "normal" (inlier) data points. Many many trees are created via this random partitioning (hence the "forest"). When novel data is fed through, the depths of its features are used to calculate the anomaly score.
...
> Note: Isolation Trees are **not** one of the TF3800 (real-time compatible) model types. Therefore, you will have to use either TF3820 Machine Learning Server, *or* find some other way... (e.g. ADS).

For Machine Learning Server:

Add to `unsupervised.py` to generate the ONNX:
```python
# generate random forest onnx file
initial_type = [('float_input', FloatTensorType([None, len(features)]))]
...
- Install Machine Learning Server via `TcPkg`
- Activate a trial license (if required)
- Follow the quick start guide [here](https://infosys.beckhoff.com/english.php?content=../content/1033/tf3820_tc3_machine_learning_server/index.html&id=)

Machine Learning Server is headless and very straight-forward. There is no configuration on the "server" side beyond generating a `JSON` file from the Model Manager. The rest is done through the client PLC interface.

> Note: I was not able to get ML Server working with Usermode runtime 🤷‍♂️
